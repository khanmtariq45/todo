the function fetch_qms_file_id is taking time and being called for two time for same file can we reduce it and achieve same functionality ?
...
def fetch_qms_file_id(filepath):
    ...
    try:
        ...
        cursor.execute("""
            SELECT TOP 1 encryptedDocId FROM QMS_DocIds_Import01 with (nolock)
            WHERE filepath LIKE ?
        """, f"%{normalized_path}%")
        row = cursor.fetchone()
        return "Not Found" if row is None else row[0]
                        ...
                        <div class="link-item">
                            ...
                            <div class="link-content">
                                ...
                                <div style="display:flex;align-items:center;gap:0.5rem;">
                                    ...
                                    <button class="copy-btn" onclick="copyToClipboard('{urllib.parse.unquote(link).replace("'", r"&#39;").replace('"', r"&quot;")}')"
                                 ...
                                 <div class="link-type">
                                    <span style="font-size:0.95em;color:#6b7280;">
                                        <b>Encrypted Doc ID:</b> {fetch_qms_file_id(link)}
                                    </span>
                                    <button class="copy-btn" onclick="copyToClipboard('#\\\\qms?DocId={fetch_qms_file_id(link)}')" title="Copy link" type="button">
                                        <i class="fas fa-copy"></i>
                                    </button>
